...
if [ "$WANT_REPORT" = "1" ]; then
    {
        ...
        [ -f "$OUT/raw/m1/pass/before.png" ] && echo "| \`raw/m1/pass/\` | NR pass on a known test card — the deciding artifact |"
        [ -f "$OUT/raw/m1/capture/before.png" ] && echo "| \`raw/m1/capture/\` | the same on the real screen (capture test) |"
